...
{
    public virtual void RunTask(DateTime nowTime, DateTime lastTime)
    {
        Log.SetFileName("AddUserRecharges");
        Log.LogDebug("AddUserRecharges Start lastTime:" + lastTime);
        Log.LogDebug("AddUserRecharges Start nowTime:" + nowTime);
    ...
    private bool AddUserRecharges(DateTime date, DateTime lastTime,TaskData data)
    {
        ...
        if(SqlManager.GetInstance().SetAndExecute(SqlCommand.SELECT_AddUserRecharges,ref resultTb,pars)<0)
        {
            return false;
        }
        bool ret = true;
...
"&equipId=" + "" +
"&platform=" + data.m_channel.Platform +
