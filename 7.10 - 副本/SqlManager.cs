...
class SqlManager
{
   ...
   @"select Channel 
     ...
     @"SELECT order_data.orderid,account.ppid,player.diamond, player.playerid,account.nickname,product.price ,order_data.timestamp  
        ...
        //某日登陆设备
        RegisterSqlCommand((int)SqlCommand.SELECT_LOGINDEVICE,
...
and PlatID=?OsId)");
        ...
        and SvrAreaId=?SvrAreaId
         and PlatID=?OsId
        and DeviceId not in
        (select DISTINCT DeviceId from PlayerRegister where date(dtEventTime)<date(?date) and Channel=?Channel and SvrAreaId=?SvrAreaId  and PlatID=?OsId )");
       ...
       @"select DISTINCT vopenid,SvrAreaId
        ...
        RegisterSqlCommand((int)SqlCommand.SELECT_NEWDEVICE,
...
");
        ...
        player,
        product,
        (SELECT ifnull(channelid,platformid)as platformid,ppid,os from account 
...
LEFT JOIN  channelLabel_lj ON account.channelLabel = channelLabel_lj.channelLabel)as account
		...
		player,
		product,
		(SELECT ifnull(channelid,platformid)as platformid,ppid,nickname,os from account 
LEFT JOIN  channelLabel_lj ON account.channelLabel = channelLabel_lj.channelLabel)as account
        WHERE order_data.playerid = player.playerid 
        AND order_data.`status`=2 
        AND player.ppid = account.ppid 
...
and account.platformid=?Channel 
and order_data.zoneid=?SvrAreaId  
and account.os =?OsId group by player.ppid)tb2 
...
LEFT JOIN  channelLabel_lj ON account.channelLabel = channelLabel_lj.channelLabel)as account 
    ...
    //0 返回值为Null
    //1 成功
    public int SetAndExecute(SqlCommand cmd, ref DataTable resultTB, MySql.Data.MySqlClient.MySqlParameter par, MySql.Data.MySqlClient.MySqlParameter[] parsArray)
    {
        resultTB.Clear();
        try
        {
