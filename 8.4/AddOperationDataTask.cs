...
public class AddOperationDataTask : EveryDayTask
{
    ...
    public bool AddOperationData(DateTime datetime)
    {
        ...
        string opNewStartDeviceNum = "0";  //新启动设备数
        ...
        opAddDeviceNum = newLogDeviceTB.Rows.Count;
        ...
        opLoginDeviceNum = logDeviceTB.Rows.Count;
        //todo启动设备= 登陆设备
        ...
        opLoginAccountNum = LogTB.Rows.Count;
        //今日新登账号数
        ...
        if (opFirstLoginAccountNum > 0)
        {
            try
            {
                ...
                for (int i = 0; i < tbResult.Rows.Count; ++i)
                {
                    ...
                    if (SqlManager.GetInstance().SetAndExecute(SqlCommand.SELECT_ACCOUNT_PAY, ref tbSum, new MySql.Data.MySqlClient.MySqlParameter("?ppid", ppid), array
        ) < 0)
                    {
                        return false;
                    }
        ...
        for (int i = 0; i < days.Length; ++i)
        {
            ...
            {
                Log.LogError("GetAccountRetention error");
            }
            else
            {
        ...
        m_postData = "opDateTime=" + gaDateTime +
                    ...
                    "&opPlatform=" + m_platform.m_platformName +
                    "&opNewStartDeviceNum=" + opNewStartDeviceNum +
